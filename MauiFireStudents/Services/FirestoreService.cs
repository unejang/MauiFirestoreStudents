...
using MauiFireStudents.Models;

namespace MauiFireStudents.Services;
...
public class FirestoreService
{
    ...
    private async Task SetupFireStore()
    {
        if (db == null)
        {
            var stream = await FileSystem.OpenAppPackageFileAsync("dx212-students-c3c97-firebase-adminsdk-hfefi-afaa5f8a0c.json");
            var reader = new StreamReader(stream);
            var contents = reader.ReadToEnd();
            db = new FirestoreDbBuilder
            {
                ProjectId = "dx212-students-c3c97",
    ...
    public async Task<List<StudentModel>> GetAllStudent()
    {
        try
        {
            ...
            var students = data.Documents.Select(doc =>
            {
                ...
                student.Id = doc.Id;
                student.IdStudent = doc.GetValue<string>("Id");
    ...
    public async Task UpdateStudent(StudentModel student)
    {
        try
        {
            ...
            var docRef = db.Collection("Samples").Document(student.Id);
